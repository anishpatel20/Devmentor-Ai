    ...
    <main className="min-h-screen bg-[#08090d] bg-[linear-gradient(rgba(255,255,255,0.035)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.035)_1px,transparent_1px)] bg-[size:40px_40px] text-white">
      <nav className="border-b border-white/10">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          ...
          >
            Already a member <span className="ml-1 text-lime-300">-&gt;</span>
          </Link>
        </div>
      </nav>
      ...
      <div className="relative mx-auto grid min-h-[calc(100vh-73px)] max-w-6xl items-center gap-16 px-6 py-14 lg:grid-cols-[1fr_420px] lg:gap-24">
        ...
        <section className="relative z-10 hidden lg:block">
          ...
          <h1 className="max-w-xl text-5xl font-semibold leading-tight tracking-tight">
            Turn questions into <span className="text-slate-500">confidence.</span>
          </h1>
          <p className="mt-6 max-w-lg text-base leading-7 text-slate-500">
            Create your workspace and learn with an AI mentor that helps you understand, debug, and build better code.
          </p>

          <div className="mt-10 max-w-md overflow-hidden rounded-xl border border-white/10 bg-[#0d0f13] shadow-2xl">
            <div className="flex items-center justify-between border-b border-white/5 px-4 py-3">
              <span className="text-[10px] text-slate-500">welcome.js</span>
            ...
            <div className="p-5 font-mono text-xs leading-7">
              <p><span className="mr-5 text-slate-700">01</span><span className="text-purple-300">const</span> <span className="text-blue-300">mentor</span> <span className="text-slate-500">= new DevMentor();</span></p>
              <p><span className="mr-5 text-slate-700">02</span><span className="text-blue-300">mentor</span><span className="text-slate-500">.explain(theWhy);</span></p>
              <p><span className="mr-5 text-slate-700">03</span><span className="text-lime-300">// build with confidence</span></p>
            </div>
          </div>
        </section>
        ...
        <section className="relative z-10 mx-auto w-full max-w-[420px] rounded-xl border border-white/10 bg-[#0d0f13]/95 p-7 shadow-2xl backdrop-blur-sm sm:p-9">
          <div className="mb-8">
            <p className="mb-3 text-xs font-medium uppercase tracking-[0.2em] text-lime-300">Get started</p>
            <h2 className="text-3xl font-semibold tracking-tight">Create your workspace</h2>
            <p className="mt-3 text-sm leading-6 text-slate-500">A smarter way to learn and ship code.</p>
          </div>
          ...
          {success && (
            <p role="status" className="mb-5 rounded-md border border-lime-300/20 bg-lime-300/5 px-3 py-2 text-sm leading-5 text-lime-200">
              {success}
            </p>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              <label htmlFor="name" className="mb-2 block text-xs font-medium text-slate-300">Your name</label>
              <input
                id="name"
                name="name"
                type="text"
            ...
            <div>
              <label htmlFor="password" className="mb-2 block text-xs font-medium text-slate-300">Create a password</label>
              <input
                id="password"
                name="password"
                type="password"
            ...
            >
              {isSubmitting ? "Creating account..." : "Create my workspace"}
            </button>
          </form>

          <p className="mt-7 text-center text-sm text-slate-500">
            Already have an account?{" "}
            <Link to="/login" className="font-medium text-lime-300 transition hover:text-lime-200">Log in</Link>
          </p>
        </section>
      </div>
    </main>
